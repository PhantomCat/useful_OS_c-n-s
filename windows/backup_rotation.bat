@echo off
REM ротация резервных копий MS SQL (.bak)

D:
CD D:\Backups

if NOT EXIST S: net use S: \\your.serv\backup
...
if %DAY% == 01 ( 
		REM Если первое число месяца - перемещаем в папку с номером месяца и префиксом monthly_
		set PREFIX=monthly
		set EXPIRE=183
		echo Сегодня 1 число, нужно сделать ежемесячную резервную копию >> %LOGFILE%
	) else if %DOW%==%BDAY% (
		REM Если Воскресенье, то переместить бекап с именем weekly_
		set PREFIX=weekly
		set EXPIRE=16
		echo Сегодня %BDAY%, нужно сделать еженедельную резервную копию >> %LOGFILE%
	) else (
		REM Производим ежедневную работу ротации
		set PREFIX=daily
		set EXPIRE=7
...
if %cd% NEQ D:\Backups (
	...
	for %%z in (*.bak) do (
		...
		) else echo Файл %BDIR%\%basedir%\%YEAR%\%MONTH%\monthly_%%z уже имеет резервную копию, пропускаю >> %LOGFILE%
			
	)
	echo forfiles -p "%BDIR%\%basedir%" -s -m %PREFIX%_*.* -d -%EXPIRE% -c "cmd /c del /q @path" >> %LOGFILE%
